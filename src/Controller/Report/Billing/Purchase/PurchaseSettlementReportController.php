...
#[AsController]
class PurchaseSettlementReportController extends AbstractController
{
    ...
    #[Route('/api/get/purchase-settlement/report', name: 'app_get_purchase_settlement_history_report')]
    public function getPurchaseSettlementHistory(Request $request, SystemSettingsRepository $systemSettingsRepository): JsonResponse
    {
        ...
        $dql = 'SELECT id, invoice_id, purchase_return_invoice_id, supplier_id, amount_pay, amount_rest FROM purchase_settlement s WHERE is_validate = 1 ';
    ...
    public function bindPurchaseSettlement(PurchaseSettlement $purchaseSettlement): array
    {
        return [
            ...
            'purchaseReturnInvoice' => $purchaseSettlement->getPurchaseReturnInvoice() ? $purchaseSettlement->getPurchaseReturnInvoice()->getId() :  '',
            'customer' => $purchaseSettlement->getSupplier() ? $purchaseSettlement->getSupplier()->getName() :  '',
            'reference' => $purchaseSettlement->getReference(),
            'totalAmount' => $purchaseSettlement->getAmountPay() + $purchaseSettlement->getAmountRest(),
            'amountPay' => number_format($purchaseSettlement->getAmountPay(), 0, null, ' '),
            'amountRest' => $purchaseSettlement->getAmountRest(),
            ...
            'paymentGateway' => $purchaseSettlement->getPaymentGateway() ? $purchaseSettlement->getPaymentGateway()->getName() :  '',
