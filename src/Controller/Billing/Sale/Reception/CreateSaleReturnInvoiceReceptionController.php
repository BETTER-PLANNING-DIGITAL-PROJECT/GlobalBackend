...
#[AsController]
class CreateSaleReturnInvoiceReceptionController extends AbstractController
{
                             ...
                             Request $request): JsonResponse
    {
        ...
        if ($existingReception){
            return new JsonResponse(['hydra:title' => 'This sale return invoice already has receive on it.'], 500);
        }
        ...
        $reception->setOriginalDocument($saleReturnInvoice->getInvoiceNumber());
        // serial number
        $reception->setReceiveAt(new \DateTimeImmutable());
        $reception->setDescription('sale return invoice draft reception');
        $reception->setIsValidate(false);
        ...
        if ($saleReturnInvoiceItems)
        {
            foreach ($saleReturnInvoiceItems as $saleReturnInvoiceItem)
            {
                ...
                $receptionItem->setInstitution($saleReturnInvoiceItem->getInstitution());
